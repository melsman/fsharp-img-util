...
do ImgUtil.runApp "Img" 600 480
           ...
           (fun (s,i) e ->
              if e.KeyCode = Keys.Up then Some (s+1,i)
              else if e.KeyCode = Keys.Down then Some (s-1,i)
              else if e.KeyCode = Keys.Right then Some (s,i+1)
              else if e.KeyCode = Keys.Left then Some (s,i-1)
              else None) (10,0)
