...
module FunImg
  ...
  let imgToBitmap (bmp:System.Drawing.Bitmap) (width:float) (img:fcolor image): unit =
    ...
    for x in [0..w-1] do
      for y in [0..h-1] do
        ...
        let fc = img (p_x,p_y)
        let c = toColor fc
        in bmp.SetPixel (x,y,c)

  let imgBitmap (width:float) (img:fcolor image) w h : System.Drawing.Bitmap =
    let format = Imaging.PixelFormat.Format24bppRgb
    let bmp = new Bitmap (w, h, format)
